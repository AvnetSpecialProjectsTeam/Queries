...
/****** Object:  Job [NascarRelease4Prod]    Script Date: 12/8/2017 11:32:10 AM ******/
EXEC msdb.dbo.sp_delete_job @job_id=N'1fee8ff8-8593-40d4-8aff-5ddc6dd16e47', @delete_unused_schedule=1
GO
...
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]    Script Date: 12/8/2017 11:32:10 AM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'NascarRelease4Prod', 
		@enabled=1, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [DBAIsPrimary]    Script Date: 12/8/2017 11:32:10 AM ******/
...
END', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [step1]    Script Date: 12/8/2017 11:32:10 AM ******/
...
update t1
set t1.COMMENTS = concat(t1.COMMENTS,''| ~NASCAR: '',''AuthNo:'',t2.AuthorizationNbr,'', ValidFrom:'',t2.ValidFrom,'', ValidTo:'',t2.ValidTo,'', Rate:'',t2.Rate,'', Qty:'',t2.RemainingQtyVistex,'', Resale:'',t2.AvnetResale)
from
...
COMMENTS
 ...
 inner join (SELECT SapAgreement, AuthorizationNbr,ValidFrom,ValidTo,SoldToParty,ShiptoParty, REPLACE(LTRIM(REPLACE(MaterialNbr, 0, '' '')), '' '', 0)  as MaterialNbr, Rate,RemainingQtyVistex,AvnetResale
FROM NascarOpenShipDebit) as t2 
 on t1.SOLD_TO_CUST=t2.SoldToParty and t1.SHIP_TO_CUST=t2.SoldToParty  and t1.MaterialNbr = t2.MaterialNbr

 
 --NASCAR SO
  -- add open SoBacklog data to comments


   update t1
set t1.COMMENTS = concat(t1.COMMENTS,''| N-SO: '',''SO#:'',t2.SalesDoc,'', Qty: '',t2.Qty,'', Value: '',t2.OrderValue,'', ExtResale: '',t2.ExtResale,'', ExtCost: '',t2.ExtCost,'', EndCust: '',t2.EndCust)
from
(select REPLACE(LTRIM(REPLACE(RIGHT(SOLD_TO_CUST,10), 0, '' '')), '' '', 0)  as SOLD_TO_CUST,
REPLACE(LTRIM(REPLACE(RIGHT(SHIP_TO_CUST,10), 0, '' '')), '' '', 0)  as SHIP_TO_CUST,
REPLACE(LTRIM(REPLACE(SAP_MATERIAL_ID, 0, '' '')), '' '', 0)  as MaterialNbr,
...
FROM NascarSoBacklog) as t2 
 ...
 --Add information from Billings for last 90 days to comment section of nascar 
 update t1
...
-- if overwrite flag is entered flag to product
 ...
 --filter out tlaa
    update [NascarProd].[dbo].NascarExportLineFinal 
  Set AUTH = ''P''
  where TLA_NO is not null;
  --filter out DesignWin
     update [NascarProd].[dbo].NascarExportLineFinal 
  Set AUTH = ''P''
  ...
  --recomment with only the new comment. This must be done because Nascar return only new comment
...
(
 ...
 select  [NascarProd].[dbo].[NascarExportLineFinal].[NASCAR_BATCH_EXPORT_ID]  AS [NASCAR_BATCH_IMPORT_ID]
	  ...
	  ,[NascarProd].[dbo].[NascarExportLineFinal].[COMMENTS]
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'schedule', 
		...
		@active_start_date=20171002, 
		@active_end_date=99991231, 
		@active_start_time=1500, 
		@active_end_time=235959, 
		@schedule_uid=N'0f260446-6999-4cdb-8aed-2c875337420e'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback

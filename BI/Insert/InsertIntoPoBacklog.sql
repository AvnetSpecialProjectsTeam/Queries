...
WHILE @cnt<2
	BEGIN
		UPDATE Bi.dbo.ImportBiPoBacklog
			SET [Column 14] =
							CASE 
								WHEN len([Column 14])<6 THEN NULL
								WHEN [Column 14] NOT LIKE '%[1-9]%' THEN NULL
								ELSE [Column 14]
							END
		SET @cnt=@cnt+1
	END
...
GO
CREATE UNIQUE CLUSTERED INDEX IndexMatNbr on #PoBlCostResale (MaterialNbr ASC)
GO
...
GO
	...
	SELECT 
	A.[Column 0] AS PoNbr 
	,A.[Column 1] AS PoLine
	,A.[Column 2] AS PoSchedLine
	,A.[Column 3] AS MaterialNbr
	,A.[Column 4] AS Plant
	,A.[Column 5] AS StorLoc
	,A.[Column 6] AS PurchOrg
	,A.[Column 8] AS mfg
	,A.[Column 7] AS ProdHrchy
	,A.[Column 9] AS RefDocNbr
	,A.[Column 10] AS PoSchedConfCd
	,A.[Column 11] AS PoItemCd
	,A.[Column 12] AS OrderDt
	,A.[Column 13] AS SchedLineDeliveryDt
	,A.[Column 14] AS ConfDlvryDt
	,A.[Column 16] AS SchedQty
	,PbCr.MinCost * A.[Column 16] AS SchedLineValue
	,A.[Column 15] AS PoOpenQty
	,CASE 
		WHEN A.[Column 8] ='TIS' THEN PbCr.MinCost * A.[Column 15]
		ELSE A.[Column 17]
	 END AS PoRemainingValue
	,PT.DocType
	,PT.PoTypeDesc
	,PT.GenericPoType
	,DATEDIFF(DAY, A.[Column 12],(CAST(GETDATE() AS DATE))) AS Age
	,DB.Category AS AgeBucket
	INTO #BiPoBlTemp
	FROM 
	(SELECT POBL.*, LEFT(POBL.[Column 0],2) AS PoLeadNbr, CASE WHEN BIConfirmationDecoder.ConfirmationType IS NULL THEN 'UnConfirmed' ELSE 'Confirmed' END AS ConfType
	FROM Bi.dbo.ImportBiPoBacklog AS POBL LEFT JOIN BIConfirmationDecoder ON POBL.[Column 10]=BIConfirmationDecoder.ConfirmationCode
	WHERE (LEFT([Column 7], 3)= '0IT' OR LEFT([Column 7], 3)= '0ST') AND [Column 6] LIKE 'H%'
	) AS A
	INNER JOIN Bi.dbo.PoType AS PT ON A.PoLeadNbr=PT.PoLeadingDigits
	LEFT JOIN #PoBlCostResale AS PbCr ON CAST(A.[Column 3] AS BIGINT)=CAST(PbCr.MaterialNbr AS BIGINT)
	,DateBuckets AS DB
	WHERE DATEDIFF(DAY, CONVERT(DATE,A.[Column 12], 103),(CAST(GETDATE() AS DATE)))>=DB.LBound AND DATEDIFF(DAY, CONVERT(DATE,A.[Column 12], 103),(CAST(GETDATE() AS DATE)))<=DB.UBound AND A.[Column 15]>0 AND PbCr.MaterialNbr IS NOT NULL
	GO
...
MERGE Bi.dbo.BiPoBacklog AS TargetTbl
USING #BiPoBlTemp AS SourceTbl
ON (TargetTbl.PoNbr=SourceTbl.PoNbr AND TargetTbl.PoLine=SourceTbl.PoLine AND TargetTbl.PoSchedLine=SourceTbl.PoSchedLine)
WHEN MATCHED 
	AND TargetTbl.SchedLineDeliveryDt <> SourceTbl.SchedLineDeliveryDt
	OR TargetTbl.SchedLineValue<> SourceTbl.SchedLineValue
	OR TargetTbl.ConfDlvryDt <> SourceTbl.ConfDlvryDt
	OR TargetTbl.PoOpenQty <> SourceTbl.PoOpenQty
	OR TargetTbl.SchedQty <> SourceTbl.SchedQty
	OR TargetTbl.PoRemainingValue <> SourceTbl.PoRemainingValue
	OR TargetTbl.PoSchedConfCd <> SourceTbl.PoSchedConfCd

THEN
	UPDATE SET
		TargetTbl.SchedLineDeliveryDt = SourceTbl.SchedLineDeliveryDt
		,TargetTbl.SchedLineValue = SourceTbl.SchedLineValue
		,TargetTbl.ConfDlvryDt = SourceTbl.ConfDlvryDt
		,TargetTbl.PoOpenQty = SourceTbl.PoOpenQty
		,TargetTbl.SchedQty = SourceTbl.SchedQty
		,TargetTbl.PoRemainingValue = SourceTbl.PoRemainingValue
		,TargetTbl.PoSchedConfCd = SourceTbl.PoSchedConfCd
...
DROP TABLE #BiPoBlTemp
TRUNCATE TABLE Bi.dbo.ImportBiPoBacklog
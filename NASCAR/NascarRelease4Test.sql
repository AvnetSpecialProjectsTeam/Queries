...
REPLACE(LTRIM(REPLACE(RIGHT(END_CUSTOMER_ACCOUNT_NO,10), 0, ' ')), ' ', 0)  as END_CUSTOMER_ACCOUNT_NO,
REPLACE(LTRIM(REPLACE(SAP_MATERIAL_ID, 0, ' ')), ' ', 0)  as MaterialNbr,
COMMENTS
 from NascarExportLineFinal) as t1
 inner join (SELECT SapAgreement, AuthorizationNbr,ValidFrom,ValidTo,SoldToParty,ShiptoParty,EndCustomer, REPLACE(LTRIM(REPLACE(MaterialNbr, 0, ' ')), ' ', 0)  as MaterialNbr, Rate,RemainingQtyVistex,AvnetResale
FROM NascarOpenShipDebit) as t2 
 on t1.SOLD_TO_CUST=t2.SoldToParty and t1.SHIP_TO_CUST=t2.SoldToParty and t1.END_CUSTOMER_ACCOUNT_NO=t2.EndCustomer  and t1.MaterialNbr = t2.MaterialNbr

...
truncate table [NascarStaging].[dbo].NascarExportHeader
insert into [NascarStaging].[dbo].NascarExportHeader
select * from [AVS92]..[IQUOTE_OWN].[NASCAR_BATCH_EXPORT]
Where READ_FL = 'N';
--insert into NASCAR_BATCH_EXPORT_LINE from NascarExportHeader Header items 
truncate table [NascarStaging].[dbo].NascarExportLine
insert into [NascarStaging].[dbo].NascarExportLine
select t1.* FROM [AVS92]..[IQUOTE_OWN].[NASCAR_BATCH_EXPORT_LINE] AS t1
  INNER JOIN [NascarStaging].[dbo].NascarExportHeader AS t2
...
update [NascarStaging].[dbo].[NascarExportHeader]
  set READ_FL='N';



 truncate table [NascarStaging].[dbo].NascarExportLineFinal
insert into [NascarStaging].[dbo].[NascarExportLineFinal] ([NASCAR_BATCH_EXPORT_ID]
      ,[QUOTE_ID]
      ,[LINE_ITEM_NO]
      ,[LINE_ITEM_STATUS]
      ,[SOLD_TO_CUST]
...
(SELECT  [NASCAR_BATCH_EXPORT_ID]
    ...
    ,round(iif(((SMPQ is not null and SMPQ <> 0) and QUANTITY_2 is not null),[SMPQ],iif(((APPROVED_MULT_2 is not null and APPROVED_MULT_2 <> 0) and QUANTITY_2 is not null), APPROVED_MULT_2, NULL)),5) as APPROVED_MULT_2
      ...
      ,round(iif(([QUANTITY_1] is not null AND ([BOOK_COST_1] is not null AND [BOOK_COST_1]<>0)),BOOK_COST_1,iif(([QUANTITY_1] is not null AND ([APPROVED_COST_1] is not null and APPROVED_COST_1 <> 0)),[APPROVED_COST_1],NULL)),5) as APPROVED_COST_1

      ,round(iif(([QUANTITY_2] is not null AND ([BOOK_COST_1] is not null AND [BOOK_COST_1]<>0)),BOOK_COST_1,iif(([QUANTITY_2] is not null AND ([APPROVED_COST_2] is not null and APPROVED_COST_2 <> 0)),[APPROVED_COST_2],NULL)),5) as APPROVED_COST_2

      ,round(iif(([QUANTITY_3] is not null AND ([BOOK_COST_1] is not null AND [BOOK_COST_1]<>0)),BOOK_COST_1,iif(([QUANTITY_3] is not null AND ([APPROVED_COST_3] is not null and APPROVED_COST_3 <> 0)),[APPROVED_COST_3],NULL)),5) as APPROVED_COST_3
     ...
     ,round(iif(((SMPQ is not null and SMPQ <>0) and (BOOK_COST_1 is not null and BOOK_COST_1<>0) and (QUANTITY_3 is not null and QUANTITY_3 <>0)),SMPQ*BOOK_COST_1,iif(((SMOQ is not null and SMOQ <>0) and (BOOK_COST_1 is not null and BOOK_COST_1<>0) and (QUANTITY_3 is not null and QUANTITY_3 <>0)),SMOQ*BOOK_COST_1,null)),5) as [APPROVED_MO_3]
	  ...
	  ,'N' As AUTH
  FROM [NascarStaging].[dbo].[NascarExportLine]) ;


--if MaterialNbr is null flag to product
  update [NascarStaging].[dbo].NascarExportLineFinal 
  Set AUTH = 'P'
...
INSERT INTO [AVS92]..[IQUOTE_OWN].[NASCAR_BATCH_IMPORT]
...
select [NascarStaging].[dbo].[NascarExportHeader].[NASCAR_BATCH_EXPORT_ID] AS [NASCAR_BATCH_IMPORT_ID]
      ,[NascarStaging].[dbo].[NascarExportHeader].[BATCH_STATUS_CD]
      ,GETDATE() as IMPORT_DT 
      ,[NascarStaging].[dbo].[NascarExportHeader].[EXPORT_LINE_QT]
      ,[NascarStaging].[dbo].[NascarExportHeader].[READ_FL]
      ,[NascarStaging].[dbo].[NascarExportHeader].[APPL_CREATE_DT]
      ,[NascarStaging].[dbo].[NascarExportHeader].[APPL_UPDATE_DT]
...
INSERT INTO [AVS92]..[IQUOTE_OWN].[NASCAR_BATCH_IMPORT_LINE]
(
	[NASCAR_BATCH_IMPORT_ID]
      ,[QUOTE_ID]
      ,[LINE_ITEM_NO]
 ...
 select  [NascarStaging].[dbo].[NascarExportLineFinal].[NASCAR_BATCH_EXPORT_ID]  AS [NASCAR_BATCH_IMPORT_ID]
      ,[NascarStaging].[dbo].[NascarExportLineFinal].[QUOTE_ID]
      ,[NascarStaging].[dbo].[NascarExportLineFinal].[LINE_ITEM_NO]
	  ,iif(AUTH_REQ like '%PT%',QUOTED_MFG,null) as QUOTED_MFG
	  ,iif(AUTH_REQ like '%PT%',QUOTED_PART,null) as QUOTED_PART
	  ,[NascarStaging].[dbo].[NascarExportLineFinal].[SAP_MATERIAL_ID]
	  ,[NascarStaging].[dbo].[NascarExportLineFinal].[QUANTITY_1]
      ,[NascarStaging].[dbo].[NascarExportLineFinal].[QUANTITY_2]
      ,[NascarStaging].[dbo].[NascarExportLineFinal].[QUANTITY_3]
	  ,[NascarStaging].[dbo].[NascarExportLineFinal].[PRICE_TYPE_1]
      ,[NascarStaging].[dbo].[NascarExportLineFinal].[PRICE_TYPE_2]
      ,[NascarStaging].[dbo].[NascarExportLineFinal].[PRICE_TYPE_3]
	  ,[NascarStaging].[dbo].[NascarExportLineFinal].[APPROVED_LT_1]
      ,[NascarStaging].[dbo].[NascarExportLineFinal].[APPROVED_LT_2]
      ,[NascarStaging].[dbo].[NascarExportLineFinal].[APPROVED_LT_3]
	  ,[NascarStaging].[dbo].[NascarExportLineFinal].[APPROVED_MIN_1]
      ,[NascarStaging].[dbo].[NascarExportLineFinal].[APPROVED_MIN_2]
      ,[NascarStaging].[dbo].[NascarExportLineFinal].[APPROVED_MIN_3]
      ,[NascarStaging].[dbo].[NascarExportLineFinal].[APPROVED_MULT_1]
      ,[NascarStaging].[dbo].[NascarExportLineFinal].[APPROVED_MULT_2]
      ,[NascarStaging].[dbo].[NascarExportLineFinal].[APPROVED_MULT_3]
	  ,[NascarStaging].[dbo].[NascarExportLineFinal].[APPROVED_COST_1]
      ,[NascarStaging].[dbo].[NascarExportLineFinal].[APPROVED_COST_2]
      ,[NascarStaging].[dbo].[NascarExportLineFinal].[APPROVED_COST_3]
	  ,[NascarStaging].[dbo].[NascarExportLineFinal].[AUTH]
	  ,[NascarStaging].[dbo].[NascarExportLineFinal].[APPL_CREATE_DT]
	  from [NascarStaging].[dbo].[NascarExportLineFinal];


	  -- create importline archive for debugging purposes 
	 Insert Into [NascarStaging].[dbo].NascarImportLineArchive ( NASCAR_BATCH_IMPORT_ID
     , [QUOTE_ID]
    ,[LINE_ITEM_NO]
	,[SAP_MATERIAL_ID]
	,[QUANTITY_1]
	...
	select  [NascarStaging].[dbo].[NascarExportLineFinal].[NASCAR_BATCH_EXPORT_ID]  AS [NASCAR_BATCH_IMPORT_ID]
      ...
      ,[NascarStaging].[dbo].[NascarExportLineFinal].[APPROVED_COST_2]
      ,[NascarStaging].[dbo].[NascarExportLineFinal].[APPROVED_COST_3]
      ,[NascarStaging].[dbo].[NascarExportLineFinal].[APPROVED_MO_1]
      ,[NascarStaging].[dbo].[NascarExportLineFinal].[APPROVED_MO_2]
      ,[NascarStaging].[dbo].[NascarExportLineFinal].[APPROVED_MO_3]
	  ,[NascarStaging].[dbo].[NascarExportLineFinal].[AUTH]
	  ,[NascarStaging].[dbo].[NascarExportLineFinal].[APPL_CREATE_DT]
	  from [NascarStaging].[dbo].[NascarExportLineFinal];
 ...
 UPDATE t1
  SET t1.READ_FL = 'Y'
  FROM [AVS92]..[IQUOTE_OWN].[NASCAR_BATCH_EXPORT] AS t1
  INNER JOIN [NascarStaging].[dbo].NascarExportHeader AS t2
  ON t1.NASCAR_BATCH_EXPORT_ID = t2.NASCAR_BATCH_EXPORT_ID;
